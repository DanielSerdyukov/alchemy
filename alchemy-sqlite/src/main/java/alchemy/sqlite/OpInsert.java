...
class OpInsert<T> implements SourceInsert<T> {

    private final SQLiteClient mClient;

    private final Collection<T> mObjects;

    OpInsert(SQLiteClient client, Collection<T> objects) {
        mClient = client;
        mObjects = objects;
    }
    ...
    public CloseableIterator<T> call() throws Exception {
        ...
        final long[] ids = mClient.insert(table, mObjects);
    ...
    public void run() {
        if (!mObjects.isEmpty()) {
            mClient.insert(mClient.resolve(mObjects), mObjects);
        }
    }
